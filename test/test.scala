...
//    println(IpUtil("222.55.127.169").addOthers.toJsonString)
    Source.fromFile("C:\\Users\\Administrator\\Desktop\\spark\\1_资料\\数据\\http.log").getLines()
      .map(
        a=>IpUtil("((25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d)))\\.){3}(25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d)))".r.findFirstIn(a).getOrElse(throw new RuntimeException("123")))
          .addRegion.addSelf.toMap)
      .flatMap(a=>a).foreach(a=>1)
  }
}